...
- **Gestión de equipo (`/dashboard/equipo`, `equipo/actions.ts`, solo admin):** el admin **crea** agentes (`createAgentAction`: `admin.auth.admin.createUser` con `email_confirm: true` + contraseña temporal que comparte; NO `signUp`, que pisaría su sesión; rollback con `deleteUser` si el insert en `agents` falla) y **elimina** agentes (`deleteAgentAction`, **Modelo B**: reasigna las propiedades del agente al admin ANTES de borrar, así nunca quedan huérfanas; después `admin.auth.admin.deleteUser` cascadea y borra la fila `agents`). ⚠ **CORRECCIÓN MEDIDA (1 sep 2026): acá decía que los leads viejos quedaban en `agent_id NULL` por una FK `ON DELETE SET NULL`. Es FALSO.** En la base, `leads.agent_id` es **NOT NULL** y su FK **no tiene cláusula ON DELETE** (o sea `NO ACTION`), así que **hoy no se puede borrar un agente que tenga consultas a su nombre**: el borrado choca contra esa FK. Y `properties.agent_id` es **NOT NULL + ON DELETE CASCADE**, con lo cual la reasignación previa del Modelo B no es una prolijidad: es lo único que impide perder las propiedades. Resolver esas dos FK es la próxima tanda (ver PENDIENTES.md). Barreras de `deleteAgentAction`: no auto-borrarse (la agencia no queda sin admin), ser admin, y que el agente target sea de la misma agencia (todo server-side). El `agency_id` del caller siempre del server. Falta (pieza siguiente): **desactivar** agente (`is_active`, reversible).
